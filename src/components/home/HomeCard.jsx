      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-5">
        {workouts.slice(0, 3).map((workout) => (
          <div key={workout.id} className="flex flex-col bg-slate border shadow-sm rounded-xl ">
            {/* <img className="w-full h-auto rounded-t-xl" src={workout.image} alt={workout.title} /> */}
            ...
            <div className="p-4 md:p-5">
              ...
              <Link className="mt-2 py-2 px-3 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-purple-600 text-white hover:bg-purple-700 disabled:opacity-50 disabled:pointer-events-none" to={`/workouts/${workout.id}`}>
                S'inscrire
              </Link>
            </div>
          </div>
        ))}
          <div className="text-center mt-8 col-start-2">
