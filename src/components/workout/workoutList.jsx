...
const WorkoutList = forwardRef(({ searchResults }, ref) => {
    ...
    const getWorkouts = async (resetVisibleCount = false) => {
        try {
            const count = resetVisibleCount ? 6 : visibleCount;
            const data = await getData(`/workouts?sort=start_date&page_size=${count}`);
            setWorkouts(data);
    ...
    useEffect(() => {
        if (!searchResults) {
            getWorkouts(true);
        } else if (searchResults === "city=&date=&time=&category_id=&participants=") {
            getWorkouts(true);
        } else {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-5">
                {workouts.slice(0, visibleCount).map((workout) =>
                    <Link key={workout.id} className="flex flex-col bg-slate border shadow-sm rounded-xl" to={`/workouts/${workout.id}`}>
                        {workout.image_url ?
                            <img className="w-full h-auto rounded-t-xl" src={workout.image_url} alt={workout.title} />
                            :
                            <img className="w-full h-auto rounded-t-xl" src="https://images.unsplash.com/photo-1599058917212-d750089bc07e?q=80&w=1169&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D" alt="Image Description" />
                        }
